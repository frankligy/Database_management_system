...
RC createPageFile (char *fileName)
{
    ...
    char *zeroM = calloc(PAGE_SIZE*sizeof(char),sizeof(char));
    fwrite(zeroM,sizeof(char),PAGE_SIZE,fp);
    fclose(fp);
    return RC_OK;
}
...
RC writeBlock(int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage)
{
    ...
    FILE *fp;
    SM_PageHandle ph;
    fp = fopen(fHandle->fileName,"r+"); //opens the file in write mode
    fseek(fp, pageNum * PAGE_SIZE, SEEK_SET); // seeks a specific page from the start of the file
...
{   
    ...
    char *zeroM = calloc (sizeof(char) * PAGE_SIZE, sizeof(char));
    fseek(fp,0,SEEK_END);
    fwrite(zeroM,sizeof(char),PAGE_SIZE,fp);
